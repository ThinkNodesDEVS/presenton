...
async def generate_image(
    prompt: str, sql_session: AsyncSession = Depends(get_async_session), request: Request = None
):
    image_prompt = ImagePrompt(prompt=prompt)
    user = getattr(request.state, "user", None) or {}
    user_id = user.get("user_id") if isinstance(user, dict) else None
    image_generation_service = ImageGenerationService(output_directory="/tmp", user_id=user_id or "public")
